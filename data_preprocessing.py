...
def remove_stopwords(texts: List[str], stop_words: List[str]) -> List[List[str]]:
    logger.info('Starting removal of stop words')
    return [[word for word in simple_preprocess(str(doc)) if word not in stop_words] for doc in texts]


def lemmatization(texts: List[str], nlp, allowed_postags: List[str] = ['NOUN', 'ADJ', 'VERB', 'ADV']) -> List[List[str]]:
...
def pre_processing(documents) -> List[List[str]]:
    ...
    print('Printing some vocabulary:\n', vectorizer.get_feature_names_out()[:10])
